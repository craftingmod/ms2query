...
export class FieldBossCmd implements Command {
  ...
  public async execute(interaction: CommandInteraction<CacheType>, bot: BotInit, tool: CommandTools) {
    ...
    const isTime = (interaction.options.getString("group") ?? "name") === "time"

    let embeds: MessageEmbed[] = []
    let attaches: MessageAttachment[] = []

    // find current time boss
    let breaked = false
    let boss: FieldBossId | -1 = -1
    for (let i = 0; i < FieldBossTime.length; i += 1) {
      const time = FieldBossTime[i]
  ...
  protected createFieldBossEmbed(boss: FieldBossId, isTime: boolean) {
    const out: Array<{ attach: MessageAttachment | null, embed: MessageEmbed }> = []
    ...
    for (let i = boss; i < FieldBoss.length; i += 1) {
      ...
      embed.setTitle(FieldBoss[i])
      embed.addField(`등장 시간`, `${(time < 0 ? "짝수 " : "")}${Math.abs(time)}분`)
      embed.addField(`위치`, FieldBossGen[i])
      embed.setColor(CommandTools.COLOR_INFO)

      if (this.images[i] != null) {
        const attach = new MessageAttachment(this.images[i]!, `${FieldBossImage[i]}.png`)
  ...
  protected createController(boss: FieldBossId | -1, sender: string, isTime: boolean) {
    ...
    for (let i = 0; i < FieldBoss.length; i += 1) {
      ...
      const btn = new MessageButton()
        ...
        .setLabel(isTime ? `${(time < 0 ? "짝수 " : "")}${Math.abs(time)}분` : FieldBoss[id])
      if (id === boss) {
